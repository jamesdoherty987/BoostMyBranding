...
const nav = [
  ...
  { href: '/dashboard/review', label: 'Review queue', icon: CheckSquare, showCount: 'pending' as const },
  { href: '/dashboard/calendar', label: 'Scheduler', icon: Calendar },
  { href: '/dashboard/generate', label: 'Generate', icon: Sparkles },
  { href: '/dashboard/personal', label: 'Personal', icon: Clapperboard },
...
export function Sidebar() {
  ...
  const Item = ({ item, onClick }: { item: (typeof nav)[number]; onClick?: () => void }) => {
    // The Overview link is /dashboard, which is a prefix of every other
    // dashboard route — so we only mark it active on an exact match.
    // Every other link matches its own subtree.
    const active =
      item.href === '/dashboard'
        ? pathname === '/dashboard'
